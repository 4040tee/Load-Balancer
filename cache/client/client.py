...
def main():
    ...
    #add a proxy argument to the command line for connecting to the cache

    parser.add_argument('-proxy',help = 'Type in a value of the form hostName:portNumber', type = str)
    ...
    #get the  proxy host name and port number from the command line argument
    try:
        proxy = args.proxy
        proxy_list = proxy.split(':') 
        proxyHost = proxy_list[0]
        proxyPort = int(proxy_list[1])
    except:
        print('Error. Invalid input for proxy option. Enter a value of the form hostName:portNumber.')    
        sys.exit(1)
    ...
    try:
        ...
        print('Error:  That host or port is not accepting connections. Make sure you are entering a proxy entry of the form hostName:PortNumber!')
        sys.exit(1)
